	...
	*/
    ...
    private void ShootMethod()
    {
        if (waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5)
        {
            ...
            {
                // Muzzle flash
                int randomNumberForMuzzleFlash = Random.Range(0, muzzelFlash.Length);
                if (bullet)
                ...
                if (muzzelFlash.Length > 0 && muzzelSpawn)
                {
                    holdFlash = Instantiate(muzzelFlash[randomNumberForMuzzleFlash], muzzelSpawn.transform.position, muzzelSpawn.transform.rotation * Quaternion.Euler(0, 0, 90)) as GameObject;
                ...
                // Raycast kiểm tra trúng vật
                RaycastHit hit;
                if (Physics.Raycast(bulletSpawnPlace.transform.position, bulletSpawnPlace.transform.forward, out hit, 100f))
                {
                    ObjectHealth objectHealth = hit.transform.GetComponent<ObjectHealth>();
                    if (objectHealth != null)
                    {
                        // Lấy damage từ bảng damage table
                        int damage = damageTable.GetDamage(currentWeaponType, objectHealth.materialType);
                        if (damage == 0) damage = 10; // default damage

                        // Gọi TakeDamage có truyền vị trí và hướng để spawn hiệu ứng
                        Vector3 hitPoint = hit.point;
                        Vector3 hitNormal = hit.normal;

                        // Sửa lại hàm TakeDamage của ObjectHealth để nhận thêm hitPoint, hitNormal
                        objectHealth.TakeDamage(damage, hitPoint, hitNormal);
                    }
                }
                ...
                waitTillNextFire = 1f / roundsPerSecond;
                bulletsInTheGun--;
            }
            else
            {
